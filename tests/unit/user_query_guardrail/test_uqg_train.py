...
@pytest.fixture
def user_query_guardrail(mock_synthex: Synthex) -> UserQueryGuardrail:
    """
    ...
    return UserQueryGuardrail(mock_synthex)
...
def test_train_calls_parent_train_with_correct_domain(
    ...
    parent_train_mock = mocker.patch.object(
        UserQueryGuardrail.__bases__[0],  # MultiLabelClassificationModel
        "train",
        return_value=TrainOutput(global_step=1, training_loss=0.1, metrics={})
    )
    
    user_query_guardrail.train(unsafe_categories=unsafe_categories, language=language)
    ...
    assert call_kwargs['domain'] == "User query safety and content moderation"
...
def test_train_passes_unsafe_categories_as_labels(
    ...
    parent_train_mock = mocker.patch.object(
        UserQueryGuardrail.__bases__[0],
        "train",
        return_value=TrainOutput(global_step=1, training_loss=0.1, metrics={})
    )
    
    user_query_guardrail.train(unsafe_categories=unsafe_categories)
...
def test_train_passes_output_path_parameter(
    ...
    user_query_guardrail.train(unsafe_categories=unsafe_categories, output_path=output_path)
...
def test_train_passes_num_samples_parameter(
    ...
    user_query_guardrail.train(unsafe_categories=unsafe_categories, num_samples=num_samples)
...
def test_train_passes_num_epochs_parameter(
    ...
    user_query_guardrail.train(unsafe_categories=unsafe_categories, num_epochs=num_epochs)
...
def test_train_passes_device_parameter(
    ...
    user_query_guardrail.train(unsafe_categories=unsafe_categories, device=device)
...
def test_train_returns_train_output(
    ...
    result = user_query_guardrail.train(unsafe_categories=unsafe_categories)
