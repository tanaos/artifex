...
class ClassificationModel(BaseModel):
    ...
    def train(
        ...
        Args:
            ...
            classes (Optional[dict[str, str]]): A dictionary mapping class names to their descriptions. The keys 
        ...
        if train_dataset_path is None and (domain is None or classes is None):
            raise ValidationError(
                message="The `domain` and `classes` parameters are required when `train_dataset_path` is not provided."
            )
        ...
        if classes:
            ...
            # Infer classes from the dataset
            try:
                df = pd.read_csv(train_dataset_path)
                # The labels are in the last column
                unique_labels = df.iloc[:, -1].unique()
                for label in unique_labels:
                    validated_classes[str(label)] = f"Inferred class: {label}"
            except Exception as e:
                raise ValidationError(
                    message=f"Could not infer classes from the dataset: {str(e)}. Please provide the `classes` parameter."
                )
