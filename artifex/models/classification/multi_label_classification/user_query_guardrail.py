...
class UserQueryGuardrail(MultiLabelClassificationModel):
    """
    A Guardrail Model for user queries using multi-label classification.
    
    This model can identify multiple safety concerns simultaneously in user-submitted text,
    outputting independent probabilities for each unsafe content category.
    """

    def __init__(self, synthex: Synthex):
        """
        Initializes the UserQueryGuardrail with multi-label classification capabilities.
        Args:
        ...
        self._system_data_gen_instr_val: list[str] = [
            "The 'text' field should contain user queries, questions, or prompts that a user might submit to an LLM.",
    ...
    def train(
        ...
        f"""
        Train the UserQueryGuardrail model to detect multiple unsafe content categories in user queries.
    ...
    def __call__(
        ...
        """
        Classify user queries for multiple unsafe content categories simultaneously.
        
        Args:
            text (str | list[str]): The user query or queries to be classified.
