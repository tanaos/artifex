...
class MultiLabelClassificationModel(BaseModel):
    ...
    def train(
        ...
        Args:
            domain (Optional[str]): A description of the domain or context for which the model is being trained.
                It is required if train_dataset_path is not provided.
            labels (Optional[dict[str, str]]): A dictionary mapping label names to their descriptions. The keys 
                (label names) must be strings with no spaces and a maximum length of 
                {config.CLASSIFICATION_CLASS_NAME_MAX_LENGTH} characters. It is required if 
                train_dataset_path is not provided.
        ...
        if train_dataset_path is None and (domain is None or labels is None):
            raise ValidationError(
                message="The `domain` and `labels` parameters are required when `train_dataset_path` is not provided."
            )
        ...
        if labels:
            ...
            try:
                ...
                for label in unique_labels:
                    validated_labels[str(label)] = f"Inferred label: {label}"
            except Exception as e:
                raise ValidationError(
                    message=f"Could not infer labels from the dataset: {str(e)}. Please provide the `labels` parameter."
                )
