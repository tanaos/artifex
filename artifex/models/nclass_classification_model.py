...
class NClassClassificationModel(ClassificationModel, ABC):
    ...
    def _parse_user_instructions(self, user_instructions: NClassClassificationInstructions) -> list[str]:
        ...
        if user_instructions.extra_instructions is not None:
            out.append(user_instructions.extra_instructions)
        
        return out
    
    def train(
        self, classes: dict[str, str], instructions: Optional[str] = None, output_path: Optional[str] = None, 
        num_samples: int = config.DEFAULT_SYNTHEX_DATAPOINT_NUM, num_epochs: int = 3, *args: Any, **kwargs: Any
    ) -> TrainOutput:
        f"""
