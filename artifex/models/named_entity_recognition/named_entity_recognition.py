...
class NamedEntityRecognition(BaseModel):
    ...
    def _perform_train_pipeline(
        ...
        Args:
            user_instructions (Optional[ParsedModelInstructions]): A list of user instruction strings to be used for generating the training dataset. It can be None if train_dataset_path is provided.
    ...
    def train(
        ...
        Args:
            named_entities (Optional[dict[str, str]]): A dictionary where keys are named entity tag names
                and values are their descriptions. It is required if train_dataset_path is not provided.
            language (str): The language to use for generating the training dataset. Defaults to "english".
            domain (Optional[str]): The domain that the model will be specialized in.
        ...
        if train_dataset_path is None and (domain is None or named_entities is None):
            raise ValidationError(
                message="The `domain` and `named_entities` parameters are required when `train_dataset_path` is not provided."
            )
        ...
        elif train_dataset_path:
            # Infer entities from the dataset
            try:
                df = pd.read_csv(train_dataset_path)
                # The labels are in the last column
                unique_labels_str = df.iloc[:, -1].unique()
                unique_tags = set()
                ...
                for tag in unique_tags:
                    validated_ner_instr[tag] = f"Inferred entity: {tag}"
            except Exception as e:
                raise ValidationError(
                    message=f"Could not infer entities from the dataset: {str(e)}. Please provide the `named_entities` parameter."
                )
