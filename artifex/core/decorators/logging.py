...
from artifex.core.models import Warning, TrainingLogEntry, TrainingErrorLogEntry, DailyTrainingAggregateLogEntry
...
def track_inference_calls(func: Callable) -> Callable:
    ...
    using the Cognitor monitoring library.

    Logs the following information via Cognitor:
    - Inference inputs and outputs
    - CPU and RAM usage
    - Input/output token counts (when a tokenizer is available on the instance)
    - Inference duration
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        # Lazily initialise a Cognitor monitor on the model instance so the
        # DB/file connection is created at most once per instance lifetime.
        if instance is not None and not hasattr(instance, "_cognitor"):
            tokenizer = getattr(instance, "_tokenizer", None)
            instance._cognitor = Cognitor(
        ...
        cognitor_instance: Cognitor = instance._cognitor  # type: ignore[union-attr]

        # Extract the primary input (first positional arg after self) for capture
        input_args = args[1:]
        input_data = input_args[0] if len(input_args) > 0 else kwargs

        with cognitor_instance.monitor() as m:
            with m.track():
                result = func(*args, **kwargs)
            m.capture(input_data=input_data, output=result)
